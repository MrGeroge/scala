...
object GraphXDemo {
  ...
  def main(args: Array[String]) {
    val conf=new SparkConf().setAppName("graph operation").setMaster("local[*]")
    ...
    //构建子图
    val subGraph = graph.subgraph(vpred = (id, attr) => attr._2 != "Missing") //取满足条件的顶点构成子图
    ...
    val graph1 = GraphLoader.edgeListFile(sc, path, canonicalOrientation, minEdgePartitions,
      StorageLevel.MEMORY_ONLY, StorageLevel.MEMORY_ONLY)  //根据文件创建图
    ...
    pageRankGraph.vertices.sortBy(_._2, false).saveAsTextFile("/user/hadoop/data/temp/graph/graph.pr") //按照节点第二个属性降序排列
